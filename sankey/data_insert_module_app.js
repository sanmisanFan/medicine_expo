...
var relayout = function(maindata,filter) {
    ...
    linkColor = d3.scale.log().domain([0.1,1,10]).range(["red","grey","blue"]);
    ...
    var link = linkset.selectAll(".link")
        .data(graph.links,function(d) {return d.parent.name + "-" + d.child.name;});
    ...
    entering
        .attr("transform", function(d) {
            try {
                d.relative = defaultPath.parent.x===d.x?
                    defaultPath.parent : defaultPath.child;
            } catch(error) {
		d.relative = d;
            }
...
entering
    ...
    relayout(maindata,filter=function(e) {
            ...
            var nodeNames = d3.keys(d3.nest().key(function (d) { return d; }).map(allLinks));

// when changing from 'i' to 'F1-' must address substring as well

            // intended is clicked
            if (d.name.substring(0,1) == '1') {
                  return e.parent==d.name || (e.child==(nodeNames.indexOf(e.child) > -1 ? nodeNames[nodeNames.indexOf(e.child)] : "") && e.parent==(nodeNames.indexOf(e.parent) > -1 ? nodeNames[nodeNames.indexOf(e.parent)] : ""));
            }
            ...
            if (d.name.substring(0,1) == '3' || d.name.substring(0,1) == '1') {
                return e.child==d.name || (e.parent==(nodeNames.indexOf(e.parent) > -1 ? nodeNames[nodeNames.indexOf(e.parent)] : "") && e.child==(nodeNames.indexOf(e.child) > -1 ? nodeNames[nodeNames.indexOf(e.child)] : ""));
            }
...
var dataToNodeList = function(data) {
    ...
    data.forEach(function (d) {
        ...
        graph.links.push({ "parent": d.parent,
                           "child": d.child,
                           "value": d.count}
                        );
    });
